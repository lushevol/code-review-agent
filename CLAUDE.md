...
| Package | Purpose |
|---------|---------|
| `ratan-code-review-agent` | Main Mastra-based code review agent — agents, workflows, evaluation |
| `agent-config-manager` | Fetches and caches agent config + prompt files from ADO repos |
| `ratan-markdown-tool` | Markdown conversion utilities (html2md, json2md, markdown2html) |
| `ratan-sonarqube-api` | SonarQube web API client for PR measures |
...
- `ratan-code-review-agent/src/mastra/index.ts` — Mastra instance, agent/workflow registration
- `ratan-code-review-agent/src/mastra/workflows/pr-review-workflow.ts` — top-level workflow definition
- `ratan-code-review-agent/src/mastra/workflows/pr-review-issues-workflow.ts` — issue review sub-workflow
- `ratan-code-review-agent/src/mastra/workflows/steps/` — individual workflow step implementations
- `ratan-code-review-agent/src/mastra/agents/` — LLM agent definitions
- `ratan-code-review-agent/src/mastra/types/index.ts` — shared Zod schemas and TypeScript types
- `ratan-code-review-agent/src/bootstrap/` — startup, PR scanning, session handling
- `ratan-code-review-agent/src/evaluation/` — evaluation types, dataset fixtures, judge agent
- `agent-config-manager/src/config.ts` — `AgentConfigClient` class for ADO config fetching/caching
- `agent-config-manager/src/types.ts` — config schema definitions
...
pnpm --filter ratan-code-review-agent dev

# Run demo (live PR scanning — has side effects)
pnpm --filter ratan-code-review-agent demo

# Run all tests
pnpm --filter ratan-code-review-agent exec vitest run
...
pnpm --filter ratan-code-review-agent mastra:build
