...
export class IssueClassificationService {
  private readonly issueClassificationInstructions = `
...
Output Format:
Return the classified issues in the following JSON format:
...
{{issues}}
  ...
  `;
  private readonly promptCompiler = Handlebars.compile(
    this.issueClassificationInstructions,
  );

  constructor(private readonly drizzleService: DrizzleService) {}

  private readonly logger = new Logger(IssueClassificationService.name, {
    timestamp: true,
  });
  ...
  > {
    this.logger.log(`Classifying ${issues.length} issues`);
    // const request: LLMRequest = {
    //   model: "gpt-5-mini",
    //   messages: [
    //     {
    //       role: "user",
    //       content: this.promptCompiler({
    ...
    //       },
    //       required: ["no", "category", "sub_category"],
    //     },
    //   },
    // };

    // const response = await this.taskClient.submitTask(request);
    ...
    try {
      const issuesWithClassifications = response.object;
      return issues.map((issue, index) => ({
        ...issue,
        category: issuesWithClassifications[index]?.category || "Uncategorized",
        sub_category:
          issuesWithClassifications[index]?.sub_category || "Uncategorized",
      }));
    } catch (error) {
      this.logger.error(
        "Error parsing issue classification LLM response:",
        response?.response,
      );
      return issues;
    }
  ...
  async retrieveAllIssues() {
    this.logger.log("Retrieving all issues");
    let index = 0;
    const pageSize = 100;
    while (true) {
      const issues = await this.drizzleService.getAllUnclassifiedIssues({
        pageNo: index,
        pageSize,
      });
      this.logger.log(
        `Queried ${issues.length} issues from database for classification.`,
      );

      if (issues.length === 0) {
        this.logger.log("No more unclassified issues found. Exiting.");
        break;
      }

      const classifiedIssues = await this.classifyIssues(
        issues.map((i) => ({
          message: i.message,
          suggestion: i.suggestion,
          suggestion_code: i.suggestionCode,
          category: i.issueCategory,
          sub_category: i.issueSubCategory,
        })),
      );
      await this.drizzleService.batchUpdateCodeReviewIssue(
        issues.map((issue, index) => ({
          id: issue.id,
          issueCategory: classifiedIssues[index]?.category || "",
          issueSubCategory: classifiedIssues[index]?.sub_category || "",
        })),
      );
  ...
  async checkNewIssuesClassification(
    issues: Array<CodeReviewIssue>,
  ): Promise<Array<CodeReviewIssue>> {
    if (issues.length === 0) {
      return [];
    }
  }
